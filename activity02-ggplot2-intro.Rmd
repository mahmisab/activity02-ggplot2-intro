...
**Response**: Each boxplot is given a different color. Additionally, each boxplot represents a different species. 
...
**Response**: Within a boxplot, you can easily identify the statistics of the data like the median, range, max, min, and outliers if the data has any. With the jitterplot, you can easily see all data points. You can also see the distribution within each species. This can also be seen in a boxplot, but this gives another point of view. 
...
**Response**:
I cannot seem to access my .md file or even see it. My rmd file looks more like code and probably not very professinal, but then again, I can;t see my .md file. 
...
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species, fill = species)) +
    geom_jitter(mapping=aes(fill=species))+
    geom_boxplot()
```
...
**Response**:
I prefer the boxplot over the jitter plot because you can see more of the sample statistics withthe boxplot on top. The jitter plot allows you to see outliers, but also makes it hard to identify the statistics that the boxplt are displaying (q1, q3, median, etc).
...
**Response**: Having them mapped within a certain type of plot can allow you to play with the graphs more in terms of aesthetic. If you listed the map in the ggplot line, then that controls all the plots shown, not individual. 
